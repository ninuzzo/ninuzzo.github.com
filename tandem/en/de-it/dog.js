...
[
 ...
 ['tra', "don't worry", "mach dir keine Sorgen", "non ti preoccupare", ["Italian: (you) not yourself preoccupy. The Italian grammatical sentence most similar to: mach dir keine Sorgen is: non farti alcuna/nessuna preoccupazione. n-essuna corresponds to k-eine (n- is the n- of non, so it is a negation). Italian uses double negation if nessuna is used along with non. If you want to avoid a double negation, you can use alcuna (any) instead of nessuna. Alcuna does not contain any n-, so you use it along with non. For masculine you have alcuno/nessuno, of course.", "German: make to you no worry, not a worry: k-=not and -eine=a, so keine means: not a. You cannot use nicht, you have to use k- to negate. Anyway, non farti nessuna preoccupazione in Italian does not mean to not worry about a danger, but it rather means you're welcome. The former meaning is just: non ti preoccupare."]],
