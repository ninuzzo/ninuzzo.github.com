...
[
 ...
 ['def', "I'm calling (up) (right now)", ["ich rufe an", "ich rufe jetzt an"], ["sto chiamando", "sto chiamando adesso", "adesso sto chiamando"], ["Instead of 'to be calling' in Italian we say 'to stay calling'. The -ando suffix corresponds to the -ing suffix, it denotes that the action (the call in this case) is in progress. If you put adesso at the beginning, you stress it and you use a angry tone, it means 'do not bother me. I am busy calling now and cannot do anything else'.", "In German if you want to express the same, you can add e.g. 'now' (jetzt, adesso), but there is no -ing form or equivalent: ich rufe jetzt an=I'm calling now."]],
 ...
 ['tra', ["who is calling?", "who calls?"], "wer ruft?", ["chi chiama?", "chi sta chiamando?"], "Remember that Italian uses inflection in questions more than German, even in those cases where it is clear from the structure and the words used that it is a question."],
